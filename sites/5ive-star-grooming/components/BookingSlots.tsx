      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            className="bg-bronze/10 hover:bg-bronze/20 border border-bronze/30 hover:border-bronze/50 rounded-full p-3 transition-all duration-300"
          >
            <ChevronLeft className="w-6 h-6 text-bronze" />
          </motion.button>
          ...
          <motion.button
            ...
            className="bg-bronze/10 hover:bg-bronze/20 border border-bronze/30 hover:border-bronze/50 rounded-full p-3 transition-all duration-300"
          >
            <ChevronRight className="w-6 h-6 text-bronze" />
          </motion.button>
        </motion.div>
        ...
        <div className="hidden lg:block">
          <motion.div
            ...
            className="bg-gray-900/50 border border-primary/20 rounded-2xl p-6 overflow-x-auto"
          >
            {/* Day Headers */}
                  ...
                  {weekDates.map((date, dateIndex) => {
                    ...
                    return (
                      <motion.button
                        ...
                        whileHover={!isPast ? { scale: 1.05, boxShadow: "0 10px 30px rgba(205, 127, 50, 0.3)" } : {}}
                        ...
                        className={`
                          ...
                          ${isPast 
                            ...
                            : "bg-gray-800/50 text-gray-300 border border-bronze/20 hover:bg-bronze hover:text-black hover:border-bronze cursor-pointer"
                          }
                        `}
                      >
                        {isPast ? "—" : "Available"}
        ...
        <div className="lg:hidden space-y-6">
          {weekDates.map((date, dateIndex) => (
            <motion.div
              ...
              className="bg-gray-900/50 border border-primary/20 rounded-2xl p-6"
            >
                  ...
                  return (
                    <motion.button
                      ...
                      className={`
                        ...
                        ${isPast 
                          ? "bg-gray-800/30 text-gray-600 cursor-not-allowed border border-gray-700/30" 
                          : "bg-gray-800/50 text-white border border-bronze/30 hover:bg-bronze hover:text-black hover:border-bronze hover:shadow-lg hover:shadow-bronze/20 cursor-pointer"
                        }
                      `}
                    >
                      {formatTime(slot.hour, slot.minute)}
